...
void EEPROM_Init(void) {
    ...
    while (EEPROM_EEDONE_R & 0x01); // Wait until not working
}
...
void UART0_Transmit(char data) {
    while (UART0_FR_R & 0x20) {};   // Wait while TX FIFO full
    UART0_DR_R = data;
}
...
void UART0IntHandler(void) {
    if (UART0_MIS_R & 0x10) {  // check RX interrupt flag = bit 4
        ...
        if (data & 0xF00) { // Check for any error flags in bits [11:8] OE = Overrun, BE = Break, PE = Parity, FE = Framing)
            if (data & 0x200) {  // Parity error bit (PE)
                UART0_Transmit('!');
            }
        } else {
